...
def track_particles(data, spatial_radius=6, time_window=3000, m_threshold=1000, overlap_threshold=100):
    ...
    for event in tqdm(data.itertuples(index=False, name='Event'), total=len(data), desc="Processing events"):
        ...
        for particle in active_particles:
            # Check distance from the new event to each point in the particle
            for p_event in particle.events:
                dist_sq = calculate_distance_sq(p_event[0], p_event[1], x, y)
                if dist_sq <= spatial_radius_squared and p_event[2] >= time - time_window:
                    if particle.mass >= overlap_threshold:
                        overlapping_particles.append(particle)
                        break  # Once we find a match, we can stop checking this particle
