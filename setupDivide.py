...
# Eigen のインクルードパス
eigen_include = "/opt/homebrew/include/eigen3"  # Homebrewでインストールした場合のパス

ext_modules = [
    Extension(
        'particle_tracking',  # モジュール名
        ['particle_trackingDivide.cpp'],  # コンパイルする C++ ファイル
        include_dirs=[pybind11.get_include(), eigen_include],  # pybind11 と Eigen のヘッダファイルのディレクトリを指定
