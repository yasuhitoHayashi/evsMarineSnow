...
for particle_id, particle_info in particle_data.items():
    ...
    # 前後5ミリ秒のイベントを抽出
    pre_window_mask = (event_times >= window_start - 5) & (event_times < window_start)
    post_window_mask = (event_times > window_end) & (event_times <= window_end + 5)
    
    pre_window_events = event_coords[pre_window_mask]
    post_window_events = event_coords[post_window_mask]
    print(pre_window_events.shape[0])
